#!/usr/bin/env python3
"""
OpenVLA LIBERO Inference Demo - Professional Version
...
class GenerateConfig:
    ...
    device: str = "cuda:0"                           # Device to use for inference
...
def run_evaluation(cfg: GenerateConfig, model, processor, log_file, task_suite, num_tasks_in_suite, resize_size, top_k: int = 1):
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        for episode_idx in tqdm.tqdm(range(cfg.num_trials_per_task)):
            ...
            for num_act_units in range(1, total_num_act_units):  # Try from 1 to 40 units
...
def main():
    ...
    print(f"Number of trials per task: {cfg.num_trials_per_task}")

    # Run evaluation
    print("[*] Starting evaluation...")
    try:
        run_evaluation(cfg, model, processor, log_file, task_suite, num_tasks_in_suite, resize_size, top_k)
    finally:
